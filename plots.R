...
#SAS <- aggregate(ASscore~interface+id, data, mean)
#nAS <- aggregate(numSelectedHooks~interface+id, data, mean))
#CAS <- aggregate(correctAnchorHasBeenSelected~interface+id, data, mean)
#comp <- merge(CAS, nAS)

comp <- merge(CAS, collapsed)
comp <- merge(comp, SAS)
comp <- merge(comp, nAS)
comp <- mutate(comp, controlStart= id %in% seq(1:6))
...
hist(subset(CAS, interface!="Control")$correctAnchorHasBeenSelected, xlab="Num Correct Anchor Selected", main="", breaks=20)
...
scatter <- scatter + geom_point(size=3, aes(color=interface, shape=id)) +  scale_shape_manual(values = c(0,1,2,15,19,17,0,1,2,15,19,17))
...
# save output to file
CairoPNG("C:/Users/Antoine/Dropbox/research/Experiment/figures/mturk/0.png",1200, 900)
...
distribution <- ggplot(subset(CAS, interface!= "Control"), aes(x=correctAnchorHasBeenSelected))
distribution <- distribution + geom_histogram(binwidth=2, color="black", fill="white")
#distribution <- distribution + geom_density(trim=FALSE)
#distribution <- distribution + facet_grid(~ block) + theme_bw()
print(distribution)
